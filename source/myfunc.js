...
import { readdirSync, statSync, unlinkSync, existsSync, rmdirSync } from "fs";
...
export const clearTmp = async () => {
    ...
    filenames.forEach(file => {
        ...
        try {
            ...
            console.error(`[GC] Gagal hapus ${file}:`, e);
        }
    });
    
    console.log(`[SYSTEM] Cache temp berhasil dibersihkan. Storage aman.`);
};

export const saveMediaMessageSmart = async (m, filename) => {
    if (m.type === 'viewOnceMessage') {
        m.msg = m.msg.message.imageMessage || m.msg.message.videoMessage || m.msg.message.audioMessage;
        m.type = m.msg.mtype;
    }
    const messageType = m.mtype || m.type;
    const mediaContent = m.msg || m;
    let mime = (mediaContent.msg || mediaContent).mimetype || '';
    
    let buffer = await downloadFromMessageContent(mediaContent, messageType.replace('Message', ''));
    let type = await fileTypeFromBuffer(buffer);
    let extension = type ? type.ext : mime.split('/')[1];
    
    filename = filename || Date.now();
    let filePath = `./temp/${filename}.${extension}`;
...
global.loadConnect = async function (t) {
  try {
    await t.newsletterFollow("120363293494889157@newsletter");
  } catch (e) {
  }
};